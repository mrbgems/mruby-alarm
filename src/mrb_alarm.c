...
static const struct mrb_data_type mrb_alarm_data_type = {
  "mrb_alarm_data", mrb_free,
};
...
static mrb_value mrb_alarm_init(mrb_state *mrb, mrb_value self)
{
  ...
  data = (mrb_alarm_data *)DATA_PTR(self);
  if (data) {
    mrb_free(mrb, data);
  }
  DATA_TYPE(self) = &mrb_alarm_data_type;
  DATA_PTR(self) = NULL;

  mrb_get_args(mrb, "s", &str, &len);
  data = (mrb_alarm_data *)mrb_malloc(mrb, sizeof(mrb_alarm_data));
  data->str = str;
  data->len = len;
  DATA_PTR(self) = data;
...
static mrb_value mrb_alarm_hello(mrb_state *mrb, mrb_value self)
{
  mrb_alarm_data *data = DATA_PTR(self);

  return mrb_str_new(mrb, data->str, data->len);
}

static mrb_value mrb_alarm_hi(mrb_state *mrb, mrb_value self)
{
